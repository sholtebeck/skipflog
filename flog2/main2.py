...
def get_player(player):
    url="http://www.owgr.com/Ranking.aspx?player="+player.replace(" ","%20")
    soup=soup_results(url)
    for row in soup.findAll('tr'):
        namelink = row.find('a')
        if namelink:
            id=namelink.get("href","=0").split("=")[1]
            name=namelink.string
            if '(' in name:
                name=name[:name.index('(')]
            rank=get_value(row.findAll('td')[0].get_text())
            points = get_value(row.findAll('td')[6].string)
            country = row.find("td",{"class","ctry"}).img.get("title")
            player={"id":id,"rank":rank,"name":name,"country": country,"points":points }
            return player 
    return {"name": player, "country": "USA", "rank":"NA","points":0}
...
def getRankings(request):
    ...
    if size:
        ranking_url+="?pageSize="+str(size)
    soup=soup_results(ranking_url)
    if soup.find("h2")
        week=soup.find("h2").string.split()[-1]
